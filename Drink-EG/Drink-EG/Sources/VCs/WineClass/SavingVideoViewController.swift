...
class SearchVideoViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    let saveclassLabel = UILabel()
    let imageView = UIImageView() // CollectionView에 image와 label 추가
    let nameLabel = UILabel()
    let saveListLabel = UILabel() 
    ...
    func setupSaveListLabelConstraints() { // Label의 제약 조건을 설정하는 함수
